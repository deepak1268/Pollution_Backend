...
AQIRouter.get("/getAQIdata",async (req,res) => {
    ...
    const response = await axios.get(`${AQI_URL}/latest/by-postal-code?postalCode=${postalCode}&countryCode=IND`,{
        headers: {
            "x-api-key" : X_API_KEY
        }
    })
...
AQIRouter.get("/getAQItrend",async (req,res) => {
    const postalCode = req.query.postalCode;
    const now = new Date();
    ...
    try{
        const response = await axios.get(`${AQI_URL}/history/by-postal-code?postalCode=${postalCode}&countryCode=IND&from=${date} 00:00:00&to=${date} ${time}`,{
            headers: {
                "x-api-key" : X_API_KEY
            }
        })
